        ...
        if self.curr_tags[0] == TAGS["transparent"].value:
            for tag in TAGS:
                if TAGS[tag].value == self.curr_tags[1]:
                    ...
                    self.h_direction = TAGS[tag].movement_arr[0]
                    self.f_direction = TAGS[tag].movement_arr[0]
                    self.roll = TAGS[tag].movement_arr[0]
                    self.pitch = TAGS[tag].movement_arr[0]
                    self.yaw = TAGS[tag].movement_arr[0]
                    break
            ...
            self.h_direction = TAGS[direction].movement_arr[0]
            self.f_direction = TAGS[direction].movement_arr[0]
            self.roll = TAGS[direction].movement_arr[0]
            self.pitch = TAGS[direction].movement_arr[0]
            self.yaw = TAGS[direction].movement_arr[0]

        self.curr_tags = [[-1, -1, -1]] + [self.curr_tags[i + 1] for i in range(len(self.curr_tags) - 1)]
...
class Pid:
    ...
    def apply_pid(self, target):
        ...
        print ("dt: " + str(dt) + " error_sum: " + str(self.error_sum) + " de: " + str(de) + " target: " + str(target))
...
# Speed variables to be given at vertain points.
...
DOWN_SPEED = 0.25  #
GENERAL_CENTERING_SPEED = 0.25  #
UP_CENTERING_SPEED = 0.7  # speeds for when drone is centering on a tag
DOWN_CENTERING_SPEED = 0.25  #

# tag variables
TAGS = {"up": Tag([0.5, 0.5, 0], 0.7, "y", [UP_SPEED, 0, 0, 0, 0, 0]),
        "down": Tag([0, 1, 0], -0.7, "y", [0, DOWN_SPEED, 0, 0, 0, 0]),
        "left": Tag([0, 0, 1], 0.7, "x", [0, 0, GENERAL_SPEED, 0, 0, 0]),
...
def tag_detected(data):
    ...
    if drone1.curr_tags[0] == TAGS["empty"].value:
        ...
        # zeroing out movement - necessary in order to not have drone continue moving until new moevment order is given
    ...
    print("drone1.currTags[0]: " + str(drone1.curr_tags[0]))
    print("drone1.currTags[1]: " + str(drone1.curr_tags[1]))
...
def flight():
    ...
    while (not rospy.is_shutdown()) and not drone1.land:
        print ("")
        print ("------------------------------------------------------------------------------------")
        print ("fDirection: " + str(drone1.f_direction) +
               "\thDirection: " + str(drone1.h_direction) +
               "\tvDirection: " + str(drone1.v_direction))
        ...
        if (drone1.curr_tags[0] != [-1, -1, -1] and
            ...
            print ("--------------------------------------------------------------")
            print ("lost tag " + str(drone1.currTags[0]) + " - going back to try and find it")
            print ("--------------------------------------------------------------")
